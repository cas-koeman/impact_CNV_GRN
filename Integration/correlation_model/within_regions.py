...
class CNVRegionFinder:
            ...
            for subtype in subtypes:
                ...
                while i < len(cnv_classes):
                    ...
                    if j - i >= min_length:
                        ...
                        # Only include regions with 2 or more neutral subtypes
                        if len(other_subtypes_neutral) >= 2:
                            region_info = {
    ...
    def process_within_region_correlations(self, expr_matrix, subcluster_df, regions, output_file):
        ...
        with open(output_file, 'w') as f:
            f.write("Region_ID\tChromosome\tStart\tEnd\tSubtype\tSelected_Neutral_Subtype\t"
                   "Remaining_Neutral_Subtypes\tGene_Count\tGene_Pairs\t"
            ...
            for region in regions:
                ...
                # Randomly select one neutral subtype to include with altered subtype
                selected_neutral = random.choice(neutral_subtypes)
                remaining_neutral = [s for s in neutral_subtypes if s != selected_neutral]
                ...
                # Altered group: affected subtype + one randomly selected neutral subtype
                altered_cells = list(set(subcluster_df[subcluster_df['subtype'].isin([subtype, selected_neutral])]['cell']) & 
                                   set(expr_matrix.columns))
                
                # Neutral group: remaining neutral subtypes
                neutral_cells = list(set(subcluster_df[subcluster_df['subtype'].isin(remaining_neutral)]['cell']) & 
                                   set(expr_matrix.columns))
                
                if len(altered_cells) < 2 or len(neutral_cells) < 2:
                    print(f"Region {region['region_id']} has insufficient cells (altered: {len(altered_cells)}, neutral: {len(neutral_cells)})")
                    continue
                
                # Get expression data for this region
                expr_altered = expr_matrix.loc[common_genes, altered_cells]
                expr_neutral = expr_matrix.loc[common_genes, neutral_cells]
                
                # Calculate correlations
                corr_altered, zero_var_pct1 = self.calculate_pairwise_correlations(expr_altered)
                corr_neutral, zero_var_pct2 = self.calculate_pairwise_correlations(expr_neutral)
                
                # Calculate statistics
                n_pairs = len(corr_altered)
                if n_pairs == 0:
                    continue
                ...
                f.write(f"{region['region_id']}\t{region['chromosome']}\t{region['start_pos']}\t{region['end_pos']}\t"
                       f"{subtype}\t{selected_neutral}\t{','.join(remaining_neutral)}\t{len(common_genes)}\t{n_pairs}\t"
...
def main():
    ...
    for sample_id in sample_ids:
        ...
        print(f"Found {len(regions)} differential regions (with >=2 neutral subtypes)")
